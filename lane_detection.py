...
def region_of_interset(image):
    ...
    polygon = np.array([
        [(10,720),(1270,720),(732,460),(546,460)]
        ])
...
if cap.isOpened():
    while True:
        ...
        if ret:
            ...
            cv2.imshow('roi',img)
            # print(img.shape)
            # x_min, x_max = 546, 460  #height
            # y_min, y_max = 732, 460  #width
            # roi_img = img[x_min:x_max, y_min:y_max]
            

            IMAGE_H = 256
            IMAGE_W = 512
            roi_img = cv2.resize(img, dsize=(IMAGE_W, IMAGE_H), interpolation=cv2.INTER_LINEAR)
            ...
            binary_img = cv2.resize(binary_img,(1280,720),interpolation=cv2.INTER_NEAREST)
            print(binary_img.shape)
